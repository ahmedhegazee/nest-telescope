...
export class TelescopeService implements OnModuleInit {
  ...
  async record(entry: TelescopeEntry): Promise<void> {
    ...
    if (result.success) {
      ...
      if (entry.type === 'exception' || entry.priority === 'critical') {
  ...
  resetCircuitBreaker(type: 'entryProcessing' | 'batchProcessing' | 'storage' | 'queries'): boolean {
    const circuitName = this.circuitBreakers[type];
    return this.circuitBreakerService.reset(circuitName);
  }

  forceOpenCircuitBreaker(type: 'entryProcessing' | 'batchProcessing' | 'storage' | 'queries'): boolean {
    const circuitName = this.circuitBreakers[type];
    return this.circuitBreakerService.forceOpen(circuitName);
  }

  forceCloseCircuitBreaker(type: 'entryProcessing' | 'batchProcessing' | 'storage' | 'queries'): boolean {
    const circuitName = this.circuitBreakers[type];
